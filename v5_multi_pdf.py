...
def load_pdfs(directory_path, recursive=True):
    """
    Load PDF documents from a specified directory, including nested directories.
    
    Args:
        directory_path (str): Path to the directory containing PDF files
        recursive (bool): Whether to search for PDFs in nested directories
        
    Returns:
        list: List of loaded document objects
    """
    print(f"Loading PDFs from {directory_path}{'(including subdirectories)' if recursive else ''}...")
    
    loader = DirectoryLoader(
        path=directory_path,
        glob="**/*.pdf" if recursive else "*.pdf",  # '**/' makes it search recursively
...
def store_in_pinecone(chunks, embeddings_model, index_name=INDEX_NAME):
    ...
    pc = Pinecone(api_key=PINECONE_API_KEY)
    index = pc.Index(host="https://embed-2-6kgtl49.svc.aped-4627-b74a.pinecone.io")
...
def main(pdf_directory, recursive=True):
    """
    Main function to run the document processing and embedding pipeline.
    ...
    Returns:
        object: Pinecone index
    """
    # Load and process documents
    documents = load_pdfs(pdf_directory, recursive)
...
if __name__ == "__main__":
    # Point to the top-level directory that contains all your PDF directories
    DATA_DIR_PATH = "pdfs" 
    
    # Create the vector store with PDFs from all nested directories
    # Set recursive=False if you only want PDFs directly in the specified directory
    index = main(DATA_DIR_PATH, recursive=True)